...
class DatabaseError extends MoleculerClientError {
  constructor(err) {
    if (err instanceof UniqueConstraintError) {
      super(
        `${err.errors[0].path} com valor ${err.errors[0].value} já existe`,
        400,
        'DUPLICITY ERROR',
        err.errors
      );
    } else if (err instanceof ValidationError) {
      super('A requisição contém campos inválidos', 400, 'INVALID FIELD', err.errors);
    } else if (err instanceof ForeignKeyConstraintError) {
      super('O ID de um relacionamento não foi encontrado', 400, 'REFERENCE NOT FOUND', err.errors);
    } else {
